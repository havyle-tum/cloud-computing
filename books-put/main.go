...
func updateBook(coll *mongo.Collection, book Book) error {
	filter := bson.M{"id": book.ID} // Match by custom ID

	update := bson.M{
		"$set": bson.M{
			"book_name":    book.BookName,
			"book_author":  book.BookAuthor,
			"book_edition": book.BookEdition,
			"book_pages":   book.BookPages,
			"book_year":    book.BookYear,
		},
	}
...
func main() {
	...
	defer func() {
		...
		e.PUT("/api/books/:id", func(c echo.Context) error {
			...
			book.ID = id // Make sure filter will match

			// Try to update the book
			err := updateBook(coll, book)
			if err != nil {
	...
	fmt.Println("Books PUT service starting on port 8083")
